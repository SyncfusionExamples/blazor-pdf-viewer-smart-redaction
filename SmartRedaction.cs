...
namespace PdfViewerSmartRedaction
{
    ...
    {
        private AzureOpenAI? openAIService;
        public void InitializeOpenAI(string key)
        {
            openAIService = new AzureOpenAI(key);
        }
        ...
        public async Task<List<String>> LoadDocument(Stream stream, string mimeType)
        {
            ...
            if (mimeType == "application/pdf")
            {
                ...
                {
                    await openAIService.Initialize(extractedText.ToArray());
        ...
        internal async Task<List<string>> GetSensitiveDataFromPDF(string text, List<string> selectedItems)
        {
            ...
            if (openAIService != null)
            {
                var answer = await openAIService.GetAnswerFromGPT(prompt);
